...
std::vector<char*> get_fields(unsigned char * flds)
{
  std::vector<char*> row_fields;
  char * p = (char *)flds, *end = (char *)flds;

  while (p != NULL)
  {
    char t[2] = { 0x1f, 0x00 };
    strsep(&end, t);
    row_fields.push_back(p);
    p = end;
  }
...
const char possible_kanji_data_fields[2][6] = { "Kanji", "kanji" };
int num_possible_kanji_data_fields = 2;

const char possible_keyword_data_fields[2][8] = { "Keyword", "keyword" };
int num_possible_keyword_data_fields = 2;
...
int main (int argc, char ** argv)
{
  ...
  if (ret != SQLITE_OK) { fprintf(stderr, "sqlite error: %d %s\n", __LINE__, sqlite3_errmsg(db)); exit(EXIT_FAILURE); }
  ...
  if (ret != SQLITE_ROW) { fprintf(stderr, "sqlite error: %d %s\n", __LINE__, sqlite3_errmsg(db)); exit(EXIT_FAILURE); }
  ...
  while (c != NULL)
  {
    ...
    if (num_notes_for_thismid > 0)
    {
      ...
      ret = sqlite3_prepare_v2(db, q, -1, &stmt2, NULL);
      if (ret != SQLITE_OK) { fprintf(stderr, "sqlite prepare error: %d %s\n", __LINE__, sqlite3_errmsg(db)); exit(EXIT_FAILURE); }
      ret = sqlite3_step(stmt2);
      if (ret != SQLITE_ROW) { fprintf(stderr, "sqlite step error: %d %s\n", __LINE__, sqlite3_errmsg(db)); exit(EXIT_FAILURE); }
  ...
  {
    int has_kanji_data_field = 0;
    int has_keyword_data_field = 0;
    for (std::vector<std::string>::iterator 
         it2 = it->second.begin() ; it2 != it->second.end() ; it2++)
    {
      for (int i = 0 ; i < num_possible_kanji_data_fields ; i++)
        if (it2->compare(possible_kanji_data_fields[i]) == 0)
          has_kanji_data_field = 1;
      for (int i = 0 ; i < num_possible_keyword_data_fields ; i++)
        if (it2->compare(possible_keyword_data_fields[i]) == 0)
          has_keyword_data_field = 1;
    }
    
    if (has_kanji_data_field && has_keyword_data_field)
    {
  ...
  {
    fprintf(stderr, "couldn't find a populated model with a '%s' field and a '%s' field\n", possible_kanji_data_fields[0], possible_keyword_data_fields[0]);
    exit(EXIT_FAILURE);
  }
  ...
  int kanji_field_index = -1;
  int keyword_field_index = -1;
  for (int i = 0 ; i < fields[mid].size() ; i++)
  {
    for (int k = 0 ; k < num_possible_kanji_data_fields ; k++)
      if (fields[mid][i].compare(possible_kanji_data_fields[k]) == 0)
        kanji_field_index = i;
    for (int k = 0 ; k < num_possible_keyword_data_fields ; k++)
      if (fields[mid][i].compare(possible_keyword_data_fields[k]) == 0)
        keyword_field_index = i;
    //if (fields[mid][i].compare("Keyword") == 0) keyword_field_index = i;
  }

  if (kanji_field_index == -1 || keyword_field_index == -1)
  {
    fprintf(stderr, "kanji field (%d) or keyword field (%d) not found\n", kanji_field_index, keyword_field_index);
    exit(EXIT_FAILURE);
  }
  // fprintf(stderr, "%s %d %d\n", mid.c_str(), kanji_field_index, keyword_field_index);

  printf("var kanji = new Array(\n");
  {
    snprintf(q, sizeof(q), "SELECT flds FROM notes WHERE mid = %s", mid.c_str());

    ret = sqlite3_prepare_v2(db, q, -1, &stmt, NULL);
    if (ret != SQLITE_OK) { fprintf(stderr, "sqlite error: %d %s\n", __LINE__, sqlite3_errmsg(db)); exit(EXIT_FAILURE); }
    if (ret == SQLITE_DONE) { fprintf(stderr, "No rows returned, perhaps this isn't a valid or full table\n"); exit(EXIT_FAILURE); }
    ...
    while ((ret = sqlite3_step(stmt)) == SQLITE_ROW)
    {
      ...
      std::vector<char*> row_fields = get_fields(temp_line);

      int kanji_index = -1;
      for (int kanji_index = 0 ; kanji_index < (sizeof(all_kanji) / sizeof(all_kanji[0])) ; kanji_index++)
        if (strncmp(all_kanji[kanji_index], row_fields[kanji_field_index], sizeof(all_kanji[0])) == 0)
          break;
      
      if (kanji_index == (sizeof(all_kanji) / sizeof(all_kanji[0])))
      {
        fprintf(stderr, "Kanji not found (%s - line %d)\n", row_fields[kanji_field_index], __LINE__);
        exit(EXIT_FAILURE);
      }

      printf("%s%s[\"%s\",\"%s\"]", 
        ((kanji_count == 0)?"":","), 
        (kanji_count != 0 && (kanji_count+1) % 10 == 0)?"\n":"", 
        row_fields[kanji_field_index], row_fields[keyword_field_index]
  ...
  sprintf(q, "SELECT r.time AS ms, n.flds, ((r.id - %lld) / 1000) AS rtime, r.ease, r.ivl "
  ...
  while ((ret = sqlite3_step(stmt)) == SQLITE_ROW)
  {
    ...
    const unsigned char * 
            flds  = sqlite3_column_text(stmt, 1);  // utf8
    ...
    char * kanji = row_fields[kanji_field_index];
